...
const PetSelectionComponent: React.FC<PetSelectionProps> = ({
  selectedPetId,
  setSelectedPetId,
  savedPets,
  user,
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      ...
                      <div className="bg-secondary/50 p-2 rounded-lg">
                        ...
                        <p className="font-semibold text-primary">{selectedPet.weight || 'Not set'}kg</p>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  const features = [
    {
      ...
      stats: '10k+ pets helped'
    },
    {
      id: 'vet-triage',
      title: 'AI Vet Assistant',
      description: 'Instant health assessments with professional veterinary AI guidance',
      icon: Stethoscope,
      gradient: 'from-primary via-primary/90 to-primary',
      accentColor: 'primary',
      benefits: ['24/7 availability', 'Instant assessment', 'Emergency guidance', 'Chat with AI vet'],
      stats: '5 free chats/day'
    }
  ];


  const handleGenerateNutritionRecommendation = async () => {
    if (!user?.uuid || !selectedPetId) {
      toast.error('Please select a pet and ensure you are logged in');
      return;
    }
            ...
            >
              ...
              >
                <div className="flex items-center space-x-2 px-4 py-2 bg-primary/10 rounded-full">
                  ...
                  <span className="text-sm text-primary">10k+ Happy Pets</span>
                </div>
                <div className="flex items-center space-x-2 px-4 py-2 bg-primary/10 rounded-full">
        ...
        {!selectedFeature ? (
          ...
          >
            {features.map((feature, index) => (
              ...
              >
                <Card
                  className="group relative overflow-hidden cursor-pointer border-0 shadow-lg hover:shadow-2xl transition-all duration-500 bg-card/50 backdrop-blur-sm"
                  onClick={() => {
                  ...
                  <CardContent className="relative pt-0">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      {feature.benefits.map((benefit, benefitIndex) => (
                        <motion.div
                          key={benefitIndex}
                          className="flex items-center space-x-2 text-sm p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors"
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-muted/30">
                      ...
                      >
                        <Button
                          className={`bg-gradient-to-r ${feature.gradient} border-0 text-white shadow-lg hover:shadow-xl transition-all duration-300`}
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedFeature(feature.id);
                            setShowWelcome(false);
