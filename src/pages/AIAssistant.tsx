...
export default function AIAssistant() {
  ...
  const [planPetId, setPlanPetId] = useState<string | null>(null);
  ...
  const getPlanPet = (): PetProfile | undefined => {
    if (!planPetId || planPetId === 'manual-entry') return undefined;
    return savedPets.find((pet) => pet.uuid === planPetId);
  };
  ...
  const handleSaveNutritionPlan = async () => {
    ...
    const planPet = getPlanPet();
    if (!planPet) {
      toast.error('Save your pet to your profile before saving a nutrition plan.');
      return;
    }

    if (!hasNutritionPlanData(recommendations)) {
      toast.error('No nutrition plan to save.');
      return;
    }
    ...
    try {
      await saveNutritionPlan({
        petUuid: planPet.uuid,
        petName: planPet.name,
        userUuid: user.uuid,
        recommendationResponse: recommendations,
        environmentDataDto: recommendations.environment ?? defaultEnvironmentData,
      });
      setIsPlanSaved(true);
      toast.success(`Nutrition plan saved for ${planPet.name}!`);
    } catch (error: any) {
      const aiError = handleAIError(error);
      toast.error(aiError.userMessage || aiError.message);
    } finally {
  ...
  const handleDiscardNutritionPlan = () => {
    setRecommendations(defaultPetCarePlan);
    setPlanPetId(null);
    setIsPlanSaved(false);
    toast.info('Nutrition plan discarded.');
  };
        ...
        >
          ...
          {!selectedFeature ? (
            ...
            >
              ...
              {/* Results */}
              <AnimatePresence>
                {hasNutritionPlanData(recommendations) ? (
                  <NutritionPlanPreview
                    plan={recommendations}
                    onPlanChange={setRecommendations}
                    editable={isPlanEditable}
                    petName={getPlanPet()?.name ?? recommendations.petProfileSummary?.name}
                    showSavePrompt={!isPlanSaved}
                    isSaving={isSavingPlan}
                    isSaved={isPlanSaved}
                    canSave={Boolean(getPlanPet())}
                    saveDisabledReason="Add this pet to your profile before saving a nutrition plan."
                    onSave={handleSaveNutritionPlan}
                    onDiscard={handleDiscardNutritionPlan}
                  />
                ) : selectedPetId && !isGenerating ? (
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                        ...
                        <div className="text-center text-muted-foreground">
                          {isSwitchingPet ? (
                            ...
                            >
                              ...
                              <p>Switching to {savedPets.find((pet) => pet.uuid === selectedPetId)?.name || 'selected pet'}...</p>
                            </motion.div>
                          ) : (
                            <>
                              <Utensils className="h-12 w-12 mx-auto mb-4 opacity-50" />
                              <p>
                                No nutrition plan yet for{' '}
                                {savedPets.find((pet) => pet.uuid === selectedPetId)?.name || 'this pet'}.
                              </p>
                              <p className="text-sm mt-1">Generate a plan above, then review and save it here.</p>
                            </>
                          )}
                        </div>
                      </CardContent>
